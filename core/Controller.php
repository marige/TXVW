 ...
 */
 ...
 {  
 	...
 	function __construct($request =null){
       ...
       function sendMailElastic($destinataire,$message,$nomsender="IZICHANGE",$mailsender="communication@izichange.com"){
        ...
        function sendMail($ArrayNameMail,$message,$object=null,$replyTo=null){ 
            if($object==null){
               $object='IZI CHANGE OPERATION';
               }
            if($replyTo==null){
                $replyTo="communication@izichange.com";
            }
            ...
            foreach($ArrayNameMail as $email=>$nom){ 
                //to avoid infinity loop  
                    ...
                    $headers .= 'From: IZI CHANGE <admin@izichange.com>' . "\r\n" .
                    'Reply-To: '.$replyTo. "\r\n";
                   ...
                   // mail($email, $subject,'Bonjour '.$name." \n". $message, $headers); 
            }
        }
   ...
   public function encrypt($data) {
    ...
    $pass_Clean = filter_var($pass, FILTER_SANITIZE_STRING);

        if (filter_var($pass_Clean, FILTER_SANITIZE_FULL_SPECIAL_CHARS) === false 
                || $pass_Clean != $pass
                ||strlen($pass)<8) 
            return FALSE;
